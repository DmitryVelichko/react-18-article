...
**Попадаем в интерфейс редактирования панели:**
1. Выбор репозитория и ветки.
2. Добавление новой метрики.
3. Запрос метрики. Запросы пишутся на языке PromQL:  
  `error_count{repo=~"$repo", branch=~"$branch"}`
    - **error_count** - название одной из метрик (мы их назначали в объекте `metrics` в предыдущем разделе). Результаты проверок попадают именно сюда.
    - **repo=~"$repo"** - все репозитории (можно указать какой-то конкретный, например **repo="pupok_vaskin/taking-over-the-world"**).
    - **branch=~"$branch"** - все ветки (можно указать конкретную ветку, например, **branch="master"**).
4. Набор различных трансформаций всех метрик (сложение, деление и т.д.).
5. Вид панели (гистаграмма, таблица и т.д.).
6. В правой секции редактирования настраивается вид панели.
7. Настройки дашборда.

![grafana](https://i.ibb.co/4ZTgNGr/edit-Panel.png)
